package club.archdev.archutilities.utils;
...
public class ClassRegistrationUtils {
    ...
    public static void loadListeners(String packageName) {
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (isListener(clazz)) {
                try {
                    ArchUtilities.getInstance().getServer().getPluginManager().registerEvents((Listener) clazz.newInstance(), ArchUtilities.getInstance());
    ...
    public static Collection<Class<?>> getClassesInPackage(String packageName) {
        ...
        CodeSource codeSource = ArchUtilities.getInstance().getClass().getProtectionDomain().getCodeSource();
